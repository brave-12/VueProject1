      ...
      this.$alert(`<img src=${url} />`, '请你微信支付', {
        ...
        beforeClose: (type, instance, done) => {
          ...
          if (type == 'cancel') {
            ...
            // 判断是否真的支付了(此时的type识别到点击的是 已支付成功 按钮)
            // 因为需要支付钱然后判断付款之后this.code == 200 才能跳转    所以可以先把条件关闭直接跳转
            // if (this.code == 200) {
              // 清除定时器
              clearInterval(this.timer)
              this.timer = null
        ...
        // .catch(()=>{});  防止了点击取消弹窗报错的问题
      }).catch(() => { });
      // 需要知道支付成功|失败  支付成功 路由跳转  支付失败 提示信息
      // 定时器没有  开启一个新的定时器
      if (!this.timer) {
