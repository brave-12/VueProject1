        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="chooseArea">
              ...
              <!-- 循环遍历配送至的下面的一个模块 也就是属性选择区域 -->
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrValueList" :key="spuSaleAttr.id">
                <!-- 售卖属性名  例如颜色 -->
                <dt class="title">{{spuSaleAttr.saleAttrName}}</dt>
                <!--  售卖属性值  例如陶瓷黑  isChecked 代表选中状态 -->
                <dd
                  changepirce="0"
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转是从a路由到b路由  这里在加入购物车,进行路由跳转之前 -->
                <!-- 发请求把购买的产品信息通过请求的形式通知服务器   服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    // 捞一下 store 的 detail 内的数据
    ...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrValueList']),
    // 给子组件的数据  防止返回的数据为空所以或一个空对象
    skuImageList () {
  ...
  methods: {
    // 产品的售卖属性值切换高亮  也就是排他思想  其他都不亮只有自己亮
    ...
    // 表单元素修改产品个数
    changeSkuNum (event) {
      // event.target.value * 1  乘1如果value里面有非字符的话结果为NaN
    ...
    // 加入购物车的回调函数
    async addShopCar () {
      // 1.发请求--将产品加入到数据库（通知服务器）  也就是相当于调用仓库中的 addOrUpdateShopCart 方法
      // 添加 await 等待仓库成功   trycatch成功执行第一局  失败执行第二句
      try {
        // 成功
        ...
        // 进行路由的跳转   并且在跳转的时候还需要将产品的信息带给下一级的路由组件
        // 也可以直接在这边传 skuInfo 但是路径比较乱   所以只传递 skuNum 过去
        // 而产品信息(比如skuInfo) 可以通过会话存储(网页关闭会消失) 因为这个数据是临时数据不需要本地存储
        // 本地存储|会话存储   一般存的是字符串  通过 JSON.stringify 传递
